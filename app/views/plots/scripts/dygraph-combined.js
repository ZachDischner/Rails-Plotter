...
Dygraph.prototype.drawGraph_ = function (v) {
    ...
    var f = [];
    var a = {};
    for (var y = E[0].length - 1; y >= 1; y--) {
        if (!this.visibility()[y - 1]) {
            continue
        }
        var z = this.attr_("labels")[y];
        var c = this.attr_("connectSeparatedPoints", y);
        var b = this.attr_("logscale", y);
        var m = [];
        ...
        if (this.dateWindow_) {
            ...
            this.boundaryIds_[y - 1] = [e, F];
            for (var u = e; u <= F; u++) {
                q.push(m[u])
            }
            m = q
        } else {
            this.boundaryIds_[y - 1] = [0, m.length - 1]
        }
        var n = this.extremeValues_(m);
        if (r) {
            for (var w = 0; w < m.length; w++) {
                val = [m[w][0], m[w][1][0], m[w][1][1], m[w][1][2]];
                m[w] = val
            }
        } else {
            if (this.attr_("stackedGraph")) {
    ...
    for (var y = 1; y < f.length; y++) {
        ...
        this.layout_.addDataset(this.attr_("labels")[y], f[y])
    }
...
Dygraph.prototype.computeYAxisRanges_ = function (a) {
    ...
    for (var h in this.seriesToAxisMap_) {
        ...
        var o = this.seriesToAxisMap_[h];
        while (g.length <= o) {
            g.push([])
        }
        g[o].push(h)
    }
    for (var t = 0; t < this.axes_.length; t++) {
        var b = this.axes_[t];
        if (!g[t]) {
            b.extremeRange = [0, 1]
        } else {
        ...
        if (t == 0 || b.independentTicks) {
            ...
            var y = b.computedValueRange[1] - b.computedValueRange[0];
            var c = [];
            for (var q = 0; q < e.length; q++) {
                var p = (e[q].v - l.computedValueRange[0]) / f;
                var u = b.computedValueRange[0] + p * y;
                c.push(u)
            }
